...
class Comments with ChangeNotifier {
  ...
  Future<void> fetchAndSetComments(String postId) async {
    ...
    var url = Uri.parse(
        'https://bediningboard-default-rtdb.firebaseio.com/comments.json?auth=$authToken');
    try {
      final response = await http.get(url);
      if (response.statusCode >= 400) {
        throw HttpException('Failed to load comments');
      }
  ...
  Future<void> deleteComment(String id) async {
    final url = Uri.parse(
        'https://bediningboard-default-rtdb.firebaseio.com/comments/$id.json?auth=$authToken');
